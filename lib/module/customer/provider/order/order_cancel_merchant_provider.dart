...
class CustomerOrderCancelMerchantProvider extends ChangeNotifier{
  // controller
  ...
  Future<void> fetchData() async {
    try {
      ...
      if (_orders.isNotEmpty) {
        DMethod.log("Load From Data (Cancel by Merchant");
        state = const OnSuccessState();
        notifyListeners();
        return;
      }
      DMethod.log("Load From API (Cancel by Merchant)");
  ...
  void onRefresh() {
    DMethod.log("OnRefresh (Cancel by Merchant)");
    _orders = [];
    state = const OnLoadingState();
    notifyListeners();
  }
