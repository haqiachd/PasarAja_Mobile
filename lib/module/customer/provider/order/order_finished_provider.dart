...
class CustomerOrderFinishedProvider extends ChangeNotifier{
  // controller
  ...
  Future<void> fetchData() async {
    try {
      ...
      if (_orders.isNotEmpty) {
        DMethod.log("Load From Data (Finished)");
        state = const OnSuccessState();
        notifyListeners();
        return;
      }
      DMethod.log("Load From API (Finished)");
      ...
      // get data
      final dataState = await _controller.tabFinished(
        email: email,
      );
  ...
  void onRefresh() {
    DMethod.log("OnRefresh (Finished)");
    _orders = [];
    state = const OnLoadingState();
    notifyListeners();
  }
