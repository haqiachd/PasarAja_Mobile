...
class OrderCancelCustomerTab extends StatefulWidget {
  const OrderCancelCustomerTab({Key? key}) : super(key: key);

  @override
  State<OrderCancelCustomerTab> createState() => _OrderCancelCustomerTabState();
}

class _OrderCancelCustomerTabState extends State<OrderCancelCustomerTab> {
  @override
  void initState() {
    super.initState();
  ...
  Future<void> _fetchData() async {
    try {
      await context.read<CustomerOrderCancelCustomerProvider>().fetchData();
    } catch (ex) {
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      onRefresh: () async {
        context.read<CustomerOrderCancelCustomerProvider>().onRefresh();
        await Future.delayed(
          Duration(seconds: PasarAjaConstant.onRefreshDelaySecond),
        );
        await _fetchData();
      },
      child: Consumer<CustomerOrderCancelCustomerProvider>(
          ...
          if (order.state is OnSuccessState) {
            ...
            if (orders.isNotEmpty) {
              ...
              return const EmptyOrder(
                "Belum ada Pesanan yang Dibatalkan Pembeli",
              );
            }
          }
      ...
      onTap: () {
        Get.to(
          OrderDetailPage(
            ...
            provider: CustomerOrderCancelCustomerProvider(),
          ),
