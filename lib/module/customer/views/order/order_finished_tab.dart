...
class OrderFinishedTab extends StatefulWidget {
  const OrderFinishedTab({Key? key}) : super(key: key);

  @override
  State<OrderFinishedTab> createState() => _OrderFinishedTabState();
}

class _OrderFinishedTabState extends State<OrderFinishedTab> {
  @override
  void initState() {
    super.initState();
  ...
  Future<void> _fetchData() async {
    try {
      await context.read<CustomerOrderFinishedProvider>().fetchData();
    } catch (ex) {
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      onRefresh: () async {
        context.read<CustomerOrderFinishedProvider>().onRefresh();
        await Future.delayed(
          Duration(seconds: PasarAjaConstant.onRefreshDelaySecond),
        );
        await _fetchData();
      },
      child: Consumer<CustomerOrderFinishedProvider>(
