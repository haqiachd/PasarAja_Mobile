...
class OrderInTakingTab extends StatefulWidget {
  const OrderInTakingTab({Key? key}) : super(key: key);
...
class _OrderInTakingTabState extends State<OrderInTakingTab> {
  ...
  Future<void> _fetchData() async {
    try {
      await context.read<CustomerOrderInTakingProvider>().fetchData();
    } catch (ex) {
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      onRefresh: () async {
        context.read<CustomerOrderInTakingProvider>().onRefresh();
        await Future.delayed(
          Duration(seconds: PasarAjaConstant.onRefreshDelaySecond),
        );
        await _fetchData();
      },
      child: Consumer<CustomerOrderInTakingProvider>(
          ...
          if (order.state is OnSuccessState) {
            ...
            if (orders.isNotEmpty) {
              ...
              return const EmptyOrder(
                "Belum ada Pesanan yang Dalam Perjalanan",
              );
            }
          }
      ...
      onTap: () {
        Get.to(
          OrderDetailPage(
            ...
            provider: CustomerOrderInTakingProvider(),
          ),
