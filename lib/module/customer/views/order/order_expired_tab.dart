...
class OrderExpiredTab extends StatefulWidget {
  const OrderExpiredTab({Key? key}) : super(key: key);
...
class _OrderExpiredTabState extends State<OrderExpiredTab> {
  ...
  Future<void> _fetchData() async {
    try {
      await context.read<CustomerOrderExpiredProvider>().fetchData();
    } catch (ex) {
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      onRefresh: () async {
        context.read<CustomerOrderExpiredProvider>().onRefresh();
        await Future.delayed(
          Duration(seconds: PasarAjaConstant.onRefreshDelaySecond),
        );
        await _fetchData();
      },
      child: Consumer<CustomerOrderExpiredProvider>(
      ...
      onTap: () {
        Get.to(
          OrderDetailPage(
            ...
            provider: CustomerOrderExpiredProvider(),
          ),
